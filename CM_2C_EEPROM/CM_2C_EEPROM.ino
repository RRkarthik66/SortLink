...
//colorcount reset
unsigned long colorCntreset = 5;
//unsigned long c2Cntreset = 5;
bool krichiCntFlag = false;
unsigned long  krichiCnt=0;
...
int e = ((c*255)+d);
color1Cnt=e;
Serial.print("color1Cnt in setup");
lcd.setCursor(3,0);
lcd.print(color1Cnt);
...
if (C1CountFlag1 == true and C1CountFlag2 == false)
{
  ...
  color1Cnt++;
  color1Cntflag++;
  Serial.print("color 1 count loop=");
  Serial.println(color1Cnt);
  lcd.setCursor(3,0);
  lcd.print(color1Cnt);
...
if (color1Cntflag >=colorCntreset and C1CountFlag1 == false)//40 //color 1 count resetted
{
  color1Cntflag = 0;
}
...
//if (color1Cnt >= colorCntreset  and flag1CopStorage1 == false)//40
//{
...
//}
if(color1Cntflag >= colorCntreset and flag1CopStorage1 == false /*flag2CopStorage1 == false*/)//40
{
...
if (color2Cntflag >=colorCntreset and C2CountFlag1 == false)//40 //color 1 count resetted
{
  color2Cntflag = 0;
}
...
if(color2Cntflag >= colorCntreset and flag1CopStorage2 == false /*flag2CopStorage2 == false*/)//40
{
...
if (digitalRead(crate2) == false and tray2check == false)//true
  {
    crate2alert = millis();
    tray2check = true;
    tray2checkflag = true;
  ...
  if (digitalRead(crate2) == true )//false
  {
    tray2check = false;
...
int a = (color1Cnt / 255);
int b = (color1Cnt-(a*color1Cnt));
//Serial.print("color1Cnt a in ADC");
//Serial.println(a);
//Serial.print("color1Cnt b in ADC");
//Serial.println(b);
...
if (digitalRead(pas) == false and ((error == false)or(errors3 == false)))
{
EEPROM.update(0,0);
EEPROM.update(1,0);
}
